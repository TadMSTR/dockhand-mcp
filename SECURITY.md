...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/dockhand-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[dockhand-mcp] Security Report`.
...
- SSRF via the `DOCKHAND_URL` configuration
- Injection in container names, labels, or filters passed to the Dockhand API
- Privilege escalation via unintended container start/stop operations
- Information disclosure through stack or container configuration responses that expose credentials
- Dependency vulnerabilities with a plausible exploitation path in dockhand-mcp's usage
